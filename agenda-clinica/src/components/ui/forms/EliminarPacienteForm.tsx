...
import { deletePaciente, getPacientes, type Paciente } from '../../../api/pacienteApi';

type Props = {
    onClose: () => void
}
export const EliminarPacienteForm = ({ onClose }: Props) => {

    const [pacientes, setPacientes] = useState<Paciente[]>();
    const fetchData = async () => {
        try {
            const allPacientes = await getPacientes();
            if (!allPacientes) throw new Error("No se encontraron pacientes");

            await setPacientes(allPacientes);
            console.log('pacientes cargados:', allPacientes);
        } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!formData.paciente) {
            console.error("Por favor, seleccione un paciente para eliminar.");
            return;
        }
        try {
            await deletePaciente(formData.paciente);
            onClose();
        } catch (error) {
    ...
    return (
        ...
        top-0 left-0 flex justify-center items-center z-[1001]'>
